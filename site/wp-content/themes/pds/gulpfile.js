...
gulp.task('watch', function () {
	...
	gulp.watch('./assets/stylesheets/src/**/*.scss', ['styles-dev', 'browser-sync-watch']);
	// gulp.watch('./assets/js/src/*.js', ['scripts-dev', 'scripts-admin', 'scripts-customizer', 'browser-sync-watch']);
	gulp.watch('./assets/js/src/*.js', ['scripts-dev', 'browser-sync-watch']);
...
gulp.task('browser-sync', function () {
	browserSync.init({
		open: 'external',
		host: 'paulodesa.adv.br.test',
		proxy: 'paulodesa.adv.br.test',
		port: 80
	});
});
...
// Task for dev
gulp.task('dev', ['styles-dev', 'scripts-dev', 'browser-sync', 'watch']);
